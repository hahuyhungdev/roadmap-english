#!/usr/bin/env python3
"""Insert '5) Debate Prompt' into all 26 session files before Reading Text."""
...
def debate_block(session_num: int) -> str:
    content = DEBATES[session_num]
    return f"""
<details open>
...
def process_file(path: Path, session_num: int) -> bool:
    text = path.read_text(encoding="utf-8")

    # Check already inserted
    if "5) Debate Prompt" in text:
        print(f"  SKIP {path.name}: already has Debate Prompt")
        return False

    # Insert debate block before <details open>\n<summary><strong>5) Reading Text</strong>
    reading_marker = '<details open>\n<summary><strong>5) Reading Text</strong></summary>'
    if reading_marker not in text:
        print(f"  ERROR {path.name}: Reading Text marker not found")
        return False

    debate = debate_block(session_num)
    # Replace "5) Reading Text" -> "6) Reading Text"
    new_text = text.replace(
        reading_marker,
        debate + '<details open>\n<summary><strong>6) Reading Text</strong></summary>'
    )
    # Replace "6) List of Questions + Ideas" -> "7) List of Questions + Ideas"
    new_text = new_text.replace(
        '<summary><strong>6) List of Questions + Ideas</strong></summary>',
        '<summary><strong>7) List of Questions + Ideas</strong></summary>'
    )
...
if __name__ == "__main__":
    changed = 0
    for i in range(1, 27):
        fname = BASE / f"session-{i:02d}.md"
        if not fname.exists():
            print(f"  MISSING {fname.name}")
            continue
        if process_file(fname, i):
            changed += 1
    print(f"\nDone. {changed}/26 files updated.")
